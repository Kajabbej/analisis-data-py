...
# 3. PROSES RANDOM SAMPLING (PANDAS)
n_sampel = 30
df_sampel = df.sample(n=n_sampel, random_state=42)
print(f"🎲 Sukses mengambil {n_sampel} sampel dari total {total_populasi} populasi.")
...
print(f"Standar Deviasi Sampel (s)    : Rp {std_deviasi_sampel:,.2f} ")
print(f"Standar Error Sampel (SE)     : Rp {std_error_sampel:,.2f} ")
print(f"Nilai T-Statistic             : {t_stat:.4f}")
print("-"*45)

print("\n📢 KESIMPULAN UNTUK SLIDE PRESENTASI:")
if -2.045 <= t_stat <= 2.045:
...
plt.title(f'Analisis Distribusi {kolom_analisis}: Populasi vs Sampel', fontsize=14)
